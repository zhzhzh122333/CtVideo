 ...
 */
public class HomepageAdapter extends ArrayAdapter<Video> implements OnScrollListener {
    ...
    public HomepageAdapter(Context context, int textViewResourceId, List<Video> objects,
                           GridView photoWall) {
    ...
    private void loadBitmaps(int firstVisibleItem, int visibleItemCount) {
        try {
            for (int i = firstVisibleItem; i <= firstVisibleItem + visibleItemCount; i++) {
                ...
                String imageUrl = videos.get(i).pic;
                Bitmap bitmap = getBitmapFromMemoryCache(imageUrl);
                if (bitmap == null) {
            ...
            // 预载入
            for (int i = 1; i < visibleItemCount; i = i > 0 ? i * -1 : i * -1 + 1) {
                if (firstVisibleItem + i < 0 || firstVisibleItem + visibleItemCount + i >= videos.size())
                    continue;

                String imageUrl = i < 0 ? videos.get(firstVisibleItem + i).pic :
                        videos.get(firstVisibleItem + visibleItemCount + i).pic;
